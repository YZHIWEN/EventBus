    ...
    JavaFile brewJava() {
        ...
        for (String tag : tags) {
            ...
            for (SubscribeMethod sm : subscribeMethods) {
                ...
                initsubscriptionMap.addCode("subscriptions.add(new Subscription(\"" + tag + "\",this," + sm.priority + "," + ThreadMode.class.getCanonicalName() + "." + sm.threadMode + " ));\n");
        ...
        MethodSpec.Builder onreceiveBuilder = MethodSpec.methodBuilder("onReceive")
                .returns(ParameterizedTypeName.get(Runnable.class))
                .addModifiers(Modifier.PUBLIC)
                .addAnnotation(Override.class)
                .addParameter(String.class, "tag")
                .addParameter(int.class, "priority")
                .addParameter(EventBundle.class, "bundle", Modifier.FINAL);
        for (Map.Entry<String, List<SubscribeMethod>> entry : tagInfoMap.entrySet()) {
            ...
            for (SubscribeMethod ti : entry.getValue()) {
                onreceiveBuilder.addCode("  if( priority == " + ti.priority + " )  return new Runnable(){  @Override public void run() {   target." + ti.methodName + "(bundle); }};\n");
            }
            onreceiveBuilder.addCode("}\n\n");
        }
        onreceiveBuilder.addCode("return null;\n");
